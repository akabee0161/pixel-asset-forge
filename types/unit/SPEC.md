...
- **盾はどのコマでも `base` の位置から動かさない。** 動かすのは剣を持つ腕と脚だけ。
  手前の腕が奥の盾の前を通るときは、覆われる部分を隠してよい（ロランの `right_atk_hit`）
- `breathe`: 頭を1px下げて首を1行潰す。剣は拳ごと固定し、刃を縮めない。
  頭が下がると、うなじが肩の服に直接接する。`hair_dark` が `cloth_shadow` に接するセルだけ輪郭にする
  （この2色は ΔE 7.7 で見分けられない。`cloth_hi` / `cloth_base` との境は輪郭なしでよい）
- `walk`: 上半身と剣は `base` のまま、脚（y=28〜30）だけを動かす
- **攻撃は振り下ろし。** `atk_wind` の刃は `base` と同じ長さ（ロランは11px）。`atk_hit` の斜めの刃はコマに
  収まる範囲で方向間の長さを揃える（ロランは約9px）
  - `atk_wind`: 剣を持つ拳をあごの高さ（ロランは y=13〜14）まで上げ、刃を真上に立てる。
    拳を頭の高さまで上げると、刃を `base` と同じ長さに保ったまま y=0 に収まらない
  - `atk_hit`: 拳を体の前の胸〜腰に下ろし、刃を前下がりの斜めにする。正面・背面は45°、
    横向きは縦2行につき横1px（45°ではコマの端で刃が短くなる）
...
横向きの `atk_wind` は剣を頭の後ろに上げるので、外接矩形が広がって中心がずれ、`off` と出るのが仕様どおりである
（体は `base` と同じ位置にある）。
